...
class UsersPage extends PaginatedPage {
  constructor(page) {
    ...
    this.firstEditButton = page.getByRole('button', { name: /editar (?:aluno|usu.rio)/i }).first();
    this.firstDeleteButton = page.getByRole('button', { name: /(?:excluir aluno|deletar usu.rio)/i }).first();
  ...
  async openEditUserByMatricula(matricula) {
    ...
    await row.getByRole('button', { name: /editar (?:aluno|usu.rio)/i }).click();
  }

  async openDeletePopupByMatricula(matricula) {
    const row = this.getRowByMatricula(matricula);
    await row.getByRole('button', { name: /(?:excluir aluno|deletar usu.rio)/i }).click();
  }

  async openPageContainingUser(matricula) {
    // A API ordena os usuários por id crescente; um usuário recém-criado tende a
    // aparecer na última página. Ir direto a ela evita varrer dezenas de páginas
    // com timeout por página e torna o USER-017 determinístico.
