...
const { MaterialsPage } = require('../../pages/materials.page');
...
test.describe('Testes de Conteúdos', () => {
  ...
  async function ensureBaseSubjectsCreated() {
    ...
    primarySubject = buildTestSubject({
      ...
      name: `[AUTO] Disciplina Conteúdo ${buildAutoSubjectSuffix()}`,
    ...
    emptySubject = buildTestSubject({
      ...
      name: `[AUTO] Disciplina Conteúdo ${buildAutoSubjectSuffix()}`,
      professor: `Professor Conteudo Empty ${buildAutoSubjectSuffix()}`,
      studentsCount: '35',
  ...
  test.afterEach(async ({ page }) => {
    await cleanupMaterials(page).catch(() => null);
    await cleanupTopics(page).catch(() => null);
    await cleanupSubjects(page).catch(() => null);
  });
