...
test.describe('Testes de Disciplinas', () => {
  ...
  test('DISC-013 - cadastro com quantidade de alunos negativa', async () => {
    ...
    await test.step('Then the system should reject the registration and keep the form open', async () => {
      const row = await findSubjectRowAcrossPages(negativeSubject.code);
      const wasCreated = await row.isVisible().catch(() => false);
      if (wasCreated) {
        await deleteSubjectByCodeIfExists(negativeSubject.code);
      }

      expect.soft(wasCreated, 'O sistema nao deveria cadastrar disciplinas com quantidade negativa.').toBeFalsy();
      await expect(subjectsPage.registerModalHeading).toBeVisible();
    });
  });
  ...
  test('DISC-014 - cadastro com quantidade de alunos igual a zero', async () => {
    ...
    await test.step('Then the system should reject the registration and keep the form open', async () => {
      const row = await findSubjectRowAcrossPages(zeroSubject.code);
      const wasCreated = await row.isVisible().catch(() => false);
      if (wasCreated) {
        await deleteSubjectByCodeIfExists(zeroSubject.code);
      }

      expect.soft(wasCreated, 'O sistema nao deveria cadastrar disciplinas com quantidade zero.').toBeFalsy();
      await expect(subjectsPage.registerModalHeading).toBeVisible();
    });
  });
  ...
  test('DISC-015 - cadastro com quantidade de alunos decimal', async () => {
    ...
    await test.step('Then the system should reject the registration and keep the form open', async () => {
      const row = await findSubjectRowAcrossPages(decimalSubject.code);
      const wasCreated = await row.isVisible().catch(() => false);
      if (wasCreated) {
        await deleteSubjectByCodeIfExists(decimalSubject.code);
      }

      expect.soft(wasCreated, 'O sistema nao deveria cadastrar disciplinas com quantidade decimal.').toBeFalsy();
      await expect(subjectsPage.registerModalHeading).toBeVisible();
    });
  });
